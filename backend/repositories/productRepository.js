...
const buildProductFilters = (filters = {}, alias = 'p') => {
  const where = [`${alias}.status = 'active'`, `${alias}.is_deleted = 0`, 's.is_active = TRUE'];
  const params = [];
...
const findById = async (id) => {
  const [products] = await db.execute('SELECT * FROM products WHERE id = ? AND is_deleted = 0 LIMIT 1', [id]);
...
const findDetailBySlug = async (slug, userId = null) => {
  const [rows] = await db.execute(
    `${baseSelect}
     WHERE p.slug = ? AND p.status = 'active' AND p.is_deleted = 0 AND s.is_active = TRUE
     LIMIT 1`,
    [slug],
  );
...
const listSellerProducts = async (storeId) => {
  const [rows] = await db.execute(
    `SELECT p.*,
       (SELECT image_path FROM product_images WHERE product_id = p.id ORDER BY is_primary DESC, sort_order ASC, id ASC LIMIT 1) AS primary_image
     FROM products p
     WHERE p.store_id = ? AND p.is_deleted = 0 ORDER BY p.created_at DESC`,
    [storeId],
  );
  return rows;
};
