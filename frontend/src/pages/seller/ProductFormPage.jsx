...
const ProductFormPage = () => {
  ...
  const [form, setForm] = useState({ name: '', category_id: '', description: '', price: '', stock: '', weight: '', weight_unit: 'g', condition: 'new', status: 'draft' });
    ...
    const load = async () => {
      ...
      try {
        ...
        if (productResponse) {
          ...
          setForm({ name: product.name || '', category_id: product.category_id || '', description: product.description || '', price: product.price || '', stock: product.stock || '', weight: product.weight || '', weight_unit: product.weight_unit || 'g', condition: product.condition || 'new', status: product.status || 'draft' });
        }
      } catch (error) {
  ...
  const validate = () => {
    ...
    if (!isEdit && Number(form.stock) < 0) return 'Stock cannot be negative.';
  ...
  const submit = async (event) => {
    ...
    const payload = { ...form, slug, primary_image_index: primaryIndex };
  ...
  return (
    <section className="rounded-2xl border bg-white p-6 shadow-sm">
      ...
      <form onSubmit={submit} className="mt-6 grid gap-5 lg:grid-cols-2">
        ...
        <label><span className="font-semibold">Initial stock</span><input type="number" disabled={isEdit} value={form.stock} onChange={(event) => update('stock', event.target.value)} className="mt-2 w-full rounded-xl border px-4 py-3 disabled:bg-slate-100" /><p className="mt-1 text-xs text-slate-500">Use Stock on the product list to change existing stock.</p></label>
        <label><span className="font-semibold">Weight</span><div className="mt-2 flex gap-2"><input type="number" value={form.weight} onChange={(event) => update('weight', event.target.value)} className="w-full rounded-xl border px-4 py-3" /><select value={form.weight_unit} onChange={(event) => update('weight_unit', event.target.value)} className="rounded-xl border px-3 py-3"><option value="g">g</option><option value="kg">kg</option><option value="lb">lb</option><option value="pcs">pcs</option><option value="ml">ml</option><option value="l">l</option></select></div></label>
        ...
        <label><span className="font-semibold">Status</span><select value={form.status} onChange={(event) => update('status', event.target.value)} className="mt-2 w-full rounded-xl border px-4 py-3"><option value="draft">Draft</option><option value="active">Active</option><option value="inactive">Inactive</option><option value="archived">Archived</option></select></label>
